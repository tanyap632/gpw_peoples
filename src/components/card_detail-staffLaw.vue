...
onMounted(async () => {
  try {
    const response = await fetch('https://f35b386a591e2f62.mokky.dev/staffLaw')
    const data = await response.json()
    item.value = data.find((el) => el.id === parseInt(route.params.id))
        document.title = item.value.name || 'СОТРУДНИКИ ОРГАНОВ ВНУТРЕННИХ ДЕЛ И ОРГАНОВ ГОСУДАРСТВЕННОЙ БЕЗОПАСНОСТИ'
    product.value = {
