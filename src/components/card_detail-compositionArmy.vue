...
onMounted(async () => {
  try {
    const response = await fetch('https://f35b386a591e2f62.mokky.dev/compositionArmy')
    ...
    document.title = item.value.name || 'ВОЛЬНОНАЁМНЫЙ СОСТАВ АРМИИ И ФЛОТА, ВОЙСК И ОРГАНОВ ВНУТРЕННИХ ДЕЛ, ОРГАНОВ ГОСУДАРСТВЕННОЙ БЕЗОПАСНОСТИ'
